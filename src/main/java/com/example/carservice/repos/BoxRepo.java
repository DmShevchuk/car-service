...
public interface BoxRepo extends JpaRepository<Box, Long>, JpaSpecificationExecutor<Box> {
    ...
    @Query(value =
            ...
            WHERE ((b.start_time <= make_time(:h, :min, 0))
                AND (make_time(:h, :min, 0) +
                 (make_interval(mins => :duration) / b.time_factor) <= b.end_time
                OR (b.end_time <= b.start_time
            ...
            WHERE
              ...
              AND ( -- Случай, при котором время новой записи находится внутри уже созданных заказов
                    (((ord.date_start + ord.time_start) <= make_timestamp(:y, :mon, :d, :h, :min, 0))
                        AND
                     ((make_timestamp(:y, :mon, :d, :h, :min, 0) +
                      (make_interval(mins => :duration) / time_factor)) <= (ord.date_end + ord.time_end)))
                      
                    OR
                    -- Случай, при котором новая запись полностью перекрывает старую
                    ...
                    OR
                    -- Случай, при котором конец новой записи заходит на существующую запись
                    (((make_timestamp(:y, :mon, :d, :h, :min, 0) <= (ord.date_start + ord.time_start))
                        AND
                    ...
                    OR
                    -- Случай, при котором начало новой записи заходит на существующую запись
                    (make_timestamp(:y, :mon, :d, :h, :min, 0) >= (ord.date_start + ord.time_start)
                        AND
            ...
            -- Вывод бокса, который окажет услугу наиболее быстро
